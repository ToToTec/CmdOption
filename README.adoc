...
CmdOption processes the commandline arguments as an Java string array starting from the first element.
...
You can also read all or some arguments from a file by prefixing the relative or absolute path of the file with an `@`.
This is useful in various situations including:

* re-use of same set of arguments
* arguments generated by another tool
* to avercode some platform specific limit regarding the maximal lenght of the commandline
