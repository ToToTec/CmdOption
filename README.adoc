...
== Characteristics of CmdOption

CmdOption gets the commandline arguments as an Java string array and processes it starting from the first element.
For each element, it checks if the found argument is a know option or command name.
If it is a known option, it starts to parse that option. If the options defines itself arguments, it also parses these arguments.
If the fount argumemt is detected as command, than CmdOptions switches into the command mode.
After CmdOption once switched into command mode, all succeeding arguments are only parsed into the scope of that command.

If the application supports parameters (non-options, declared with a `@CmdOption` annotation without a `names` parameter) it will scan all commandline arguemnts that are not detected as options or commands into that parameter.
